...
const MenuItemCollapsed = ({ item }) => {
  ...
  return (
    <>
      {!item.dropDown ? (
        <>
          <div id={item.id} className={`menu-item menu-item-transparent ${active === item.id ? "menu-item-active" : ""} menu-item-hoverable mb-2`} style={{ height: "40px" }}>
            <Link onClick={handleMenuItemClick} to={item.path} className="h-full w-full flex items-center">
        ...
        >
          <div className="dropdown-toggle">
            <div className="menu-collapse-item menu-collapse-item-transparent mb-2" style={{ height: "40px" }}>
              <span className="text-2xl mr-2">
                {item.icon}
              </span>
            </div>
          ...
          {show && (
            <ul 
              ...
              style={show ? {opacity: "1", visibility: "visible", transform: "rotateX(0deg)" } : { }}
            >
              {item.subMenu.map((item, index) => {
                return (
                  <MenuSubItemCollapsed key={index} item={item} />
                )
              })}
            </ul>
          )}
