...
const MobileMenuItem = ({ item }) => {
  ...
  return (
    <>
      {!item.dropDown ? (
        <div className={`menu-item menu-item-transparent ${active === item.id ? "menu-item-active" : ""} menu-item-hoverable mb-2`} style={{ height: "40px" }}>
          <Link to={item.path} onClick={handleMenuItemClick} className="h-full w-full flex items-center">
        ...
        >
          ...
          {menuOpen && item.subMenu && (
            <ul
              className="ml-5"
              style={menuOpen ? { opacity: "1", height: "auto", overflow: "hidden" } : { opacity: "0", height: "0px", overflow: "hidden" }}
            >
              {item.subMenu.map((item, index) => {
                return (
                  <MobileMenuSubItem key={index} item={item} />
                );
              })}
            </ul>
          )}
